...
void read_lambda_values(int digit)
{
	FILE *fpt;
	char filename[40];
	sprintf(filename,"lambda_values/234101060_E_%d_pi.txt",digit);
	fpt=fopen(filename,"r");
	for(int i=1;i<=N;i++)
		fscanf(fpt,"%lf",&pi[i]);
	fclose(fpt);
	sprintf(filename,"lambda_values/234101060_E_%d_A.txt",digit);
	fpt=fopen(filename,"r");
	for(int i=1;i<=N;i++)
	{
		for(int j=1;j<=N;j++)
	...
	fclose(fpt);
	sprintf(filename,"lambda_values/234101060_E_%d_B.txt",digit);
	fpt=fopen(filename,"r");
	for(int i=1;i<=N;i++)
	{
		for(int j=1;j<=M;j++)
...
void training()
{
	...
	for(int i=0;i<=9;i++)
	{
		...
		printf("training_sample/234101060_E_%d\n",i);
		for(int j=1;j<=sample_count;j++)
		{
			sprintf(filename,"training_sample/234101060_E_%d_%d.txt",i,j);
